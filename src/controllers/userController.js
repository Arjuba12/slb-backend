...
const getAll = async (req, res) => {
    try {
        ...
        let query = `
            SELECT u.id, u.nama, u.email, u.role, u.no_hp, u.is_aktif, u.last_login, u.login_ip,
            ...
            LEFT JOIN kelas_guru kg ON kg.guru_id = g.id
            LEFT JOIN kelas k ON k.id = kg.kelas_id
            WHERE 1=1
        `;
        const params = [];
        ...
        query += ' GROUP BY u.id ORDER BY u.role, u.nama';
...
const update = async (req, res) => {
    try {
        ...
        await db.execute(
            'UPDATE users SET nama=?, no_hp=?, is_aktif=? WHERE id=?',
            [nama, no_hp, is_aktif, req.params.id]
        );

        const [guruRows] = await db.execute('SELECT id FROM guru WHERE user_id = ?', [req.params.id]);
        if (guruRows.length) {
            await db.execute(
...
const getGuru = async (req, res) => {
    try {
        const [rows] = await db.execute(`
            SELECT u.id, u.nama, u.email, u.no_hp, u.is_aktif,
                   ...
                   (SELECT COUNT(*) FROM perkembangan_harian ph WHERE ph.guru_id = g.id 
                    AND ph.tanggal = CURDATE()) AS input_hari_ini,
                   (SELECT COUNT(DISTINCT s.id) FROM siswa s 
                    JOIN kelas k2 ON k2.id = s.kelas_id 
                    JOIN kelas_guru kg2 ON kg2.kelas_id = k2.id AND kg2.guru_id = g.id) AS total_siswa
            FROM users u
    ...
    try {
        ...
        const [rows] = await db.execute(`
            SELECT u.nama AS nama_guru,
                   GROUP_CONCAT(DISTINCT k.nama_kelas ORDER BY k.nama_kelas SEPARATOR ', ') AS kelas,
                   COUNT(DISTINCT ph.id) AS total_input,
                   COUNT(DISTINCT s.id) AS total_siswa,
                   ROUND(COUNT(DISTINCT ph.id) / (COUNT(DISTINCT s.id) * 5) * 100, 0) AS persen_tepat_waktu,
                   MAX(ph.tanggal) AS input_terakhir,
                   CASE WHEN COUNT(DISTINCT ph.id) / (COUNT(DISTINCT s.id) * 5) >= 0.9 THEN 'Baik'
                        WHEN COUNT(DISTINCT ph.id) / (COUNT(DISTINCT s.id) * 5) >= 0.6 THEN 'Cukup'
            ...
            LEFT JOIN perkembangan_harian ph ON ph.guru_id = g.id
