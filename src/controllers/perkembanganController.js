...
const getBySiswa = async (req, res) => {
    try {
        ...
        let query = `
            SELECT ph.*, ap.nama AS aspek_nama, ap.kode AS aspek_kode,
                   u.nama AS nama_guru
...
const getRingkasanSiswa = async (req, res) => {
    try {
        ...
        const [rows] = await db.execute(`
            ...
            GROUP BY bulan_key, bulan_label, ap.id, ap.nama, ap.kode
            ORDER BY bulan_key, ap.id
        `, [siswaId]);
        ...
        const [trendRows] = await db.execute(`
            ...
            LEFT JOIN perkembangan_harian ph ON ph.aspek_id = ap.id AND ph.siswa_id = ?
            WHERE ap.is_aktif = 1
...
const create = async (req, res) => {
    try {
        ...
        const [existing] = await db.execute(
            'SELECT id FROM perkembangan_harian WHERE siswa_id = ? AND tanggal = ? AND aspek_id = ? LIMIT 1',
...
const getRekapKelas = async (req, res) => {
    try {
        ...
        const [rows] = await db.execute(`
            SELECT s.id AS siswa_id, s.nama,
            ...
            LEFT JOIN aspek_perkembangan ap ON ap.id = ph.aspek_id
    ...
    try {
        ...
        const [rows] = await db.execute(`
            ...
            LEFT JOIN perkembangan_harian ph ON ph.siswa_id = s.id
                AND ph.tanggal >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)
            LEFT JOIN aspek_perkembangan ap ON ap.id = ph.aspek_id
            LEFT JOIN absensi a_abs ON a_abs.siswa_id = s.id
